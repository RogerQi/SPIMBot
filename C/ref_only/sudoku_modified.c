...
void init_board(const char *board_init[GRID_SQUARED], unsigned short board[GRID_SQUARED][GRID_SQUARED]) {
    for (int i = 0 ; i < GRID_SQUARED ; ++ i) {
...
void print_board_mips(unsigned short board[GRID_SQUARED][GRID_SQUARED]) {
    for (int i = 0 ; i < GRID_SQUARED ; ++ i) {
        printf(".half ");
        for (int j = 0 ; j < GRID_SQUARED ; ++ j) {
            printf(" %d", board[i][j]);
...
void print_board_verbose(unsigned short board[GRID_SQUARED][GRID_SQUARED]) {
    for (int i = 0 ; i < GRID_SQUARED ; ++ i) {
        for (int j = 0 ; j < GRID_SQUARED ; ++ j) {
            unsigned value = board[i][j];
            char c = ' ';
...
bool rule1(unsigned short board[GRID_SQUARED][GRID_SQUARED], int* skip) {
    bool changed = false;
    for (int i = 0 ; i < GRID_SQUARED ; ++ i) {
        if( (1 << i) & *skip ){
...
bool rule2(unsigned short board[GRID_SQUARED][GRID_SQUARED], int* skip) {
    bool changed = false;
    for (int i = 0 ; i < GRID_SQUARED ; ++ i) {
        if( (1 << i) & *skip ){
            //printf("skipping row %d\n", i);
...
bool board_failed(unsigned short board[GRID_SQUARED][GRID_SQUARED]) {
    for (int i = 0 ; i < GRID_SQUARED ; ++ i) {
        for (int j = 0 ; j < GRID_SQUARED ; ++ j) {
            if (board[i][j] == 0) {
                return true;
...
bool solve(unsigned short board[GRID_SQUARED][GRID_SQUARED]) {
    ...
    bool changed;
    do {
        changed = rule1(board, skip);
        changed |= rule2(board, skip);
        step ++;
    } while (changed);
